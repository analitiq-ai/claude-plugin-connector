...
- `storage-connector-creator` — decline stub for every kind the plugin recognizes but cannot yet author: `kind ∈ {file, s3, stdout}` (engine does not execute them) and `kind ∈ {nosql, document}` (share the `database-endpoint` schema but have no SQL authoring path in the SQL-only `db-connector-creator`). Returns a structured refusal until a real authoring path lands.
...
- **Connector:** Reusable provider transport + auth contract. Lives in `{connector_id}/definition/connector.json` and validates against `https://schemas.analitiq.ai/connector/latest.json`. Top-level fields: `$schema`, `kind` (one of `api`, `database`, `nosql`, `document`, `file`, `s3`, `stdout`; only `api` and `database` have an authoring path — the rest are declined), `connector_id` (the stable slug, matching `[a-z0-9_-]+`; the on-disk directory uses the same value), `version`, `default_transport`, `transports`, `auth`, `connection_contract`, optional `resource_discovery`. Registry-stamped fields (`created_at`, `updated_at`) must NOT appear in authored documents.
