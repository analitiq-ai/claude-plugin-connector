...
- `connector-schema-validator` — runs structural (contract-model) plus semantic validators (reserved-field, expression-resolver, phase-resolvability, transport-ref, dsn-binding, auth-shape, tls-consistency, type-map-coverage, type-map-rule, type-map-write-coverage, endpoint-annotations, endpoint-filename). JSON documents only — package files are registry CI's job. Backed by the published **`analitiq-validator`** package (offline, model-driven), which the agent self-installs at runtime; registry CI installs the same package to run the semantic checks standalone.
...
Authored documents declare `$schema` with this host — the URL is locked by a `const` inside each schema. The validator matches on this URL offline; it does not fetch it.

The `connector-schema-validator` sub-agent runs the published **`analitiq-validator`** package (which the agent self-installs at runtime), performing single-document contract-model validation (structure **and** every cross-field rule) plus semantic validators (reserved-field, expression-resolver, phase-resolvability, transport-ref, dsn-binding, auth-shape, tls-consistency, type-map-coverage, type-map-rule, type-map-write-coverage, endpoint-annotations, endpoint-filename). Validation is **offline and model-driven** — no schema fetch. The validator checks JSON documents only — the database package files are enforced by registry CI. The console entry point is `analitiq-validate`; registry CI installs the same package to run the semantic checks outside the plugin runtime. The published schema (generated from the same contract models) is the single source of truth. The plugin's own drift guard lives under `tests/schema_drift/`.
...
- **Drift-check CI** for anything that must stay duplicated as decision logic (e.g. the `enum-mappers` that map provider facts onto schema enums): `tests/schema_drift/test_schema_drift.py` loads the live schemas and fails the build if the plugin's enum targets diverge.
