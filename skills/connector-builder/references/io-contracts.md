      ...
      "properties": {
        ...
        "resources": {
          ...
          "items": {
            ...
            "required": ["key"],
            "properties": {
              "key": { "type": "string", "description": "Stable resource id — becomes the endpoint_id and the on-disk filename `endpoints/{key}.json` (pattern ^[a-z0-9][a-z0-9_-]*$). DERIVE it, do not free-pick it: flatten the resource's full locator, lowercased, with `__` between path levels so the full path — not just the leaf — distinguishes the id. API: version segment first, then every path segment, with path-params dropped (they are operation-level) — `/v1/blah/something/customer` → `v1__blah__something__customer`, `/v2/blah/something/customer` → `v2__blah__something__customer`. Deterministic: the same locator yields the same key on every re-author. (Database resources are not authored here — their `schema__table` form is produced by runtime `resource_discovery`.)" },
              "label": { "type": "string" },
              "method": { "type": "string" },
              "path": { "type": "string" },
