"""
Endpoint schema models, aligned with the docs under the published Analitiq schema documentation:

  * the published Analitiq schema documentation (shared shape)
  * the published Analitiq schema documentation (api)
  * the published Analitiq schema documentation (database)
...
canonicalization belong to the published Analitiq schema documentation and the
runtime layer; they are not fields on the endpoint document.
...
Stream-side endpoint references (``EndpointRef``) live in ``k2m.models.stream``.
"""
import re
...
from k2m.models.shared.common import schema_url_for
...
# §Top-Level Fields — `endpoint_id` matches `[a-z0-9][a-z0-9_-]*`.
ENDPOINT_ID_PATTERN = r"^[a-z0-9][a-z0-9_-]*$"
PATH_PLACEHOLDER_NAME_PATTERN = r"^[a-z][a-z0-9_]*$"
# Spec: the published Analitiq schema documentation & api-endpoint §Filters —
# segment spelling and casing are preserved. Pattern only enforces the dotted
# non-empty-segment shape; identifier chars are provider-owned.
...
# (k2m.models.stream.ArrowFieldSpec, k2m.models.endpoints.Column) and at the
...
ENDPOINT_ID_RE = re.compile(ENDPOINT_ID_PATTERN)
...
class _EndpointModel(BaseModel):
    """Shared base model — closed contract, rejects all unknown keys.

    Per the published Analitiq schema documentation (closed contract),
    no `x-*` extensions are accepted; provider extensions must be first-class
    fields. Pydantic's `extra="forbid"` surfaces the rejection.
    ...
    def _reject_reserved_fields(cls, data: Any) -> Any:
        ...
        Per `endpoint-schema-parameterization.md` §Reserved Fields, reserved
        names (`connector_id`, `connector_version`, `connection_id`,
...
def _expression_discriminator(v: Any) -> Optional[str]:
    ...
    if isinstance(v, RefExpr):
        return "ref"
    if isinstance(v, TemplateExpr):
        return "template"
    if isinstance(v, LiteralExpr):
        return "literal"
...
Expression = Annotated[
    Union[
        Annotated[RefExpr, Tag("ref")],
        Annotated[TemplateExpr, Tag("template")],
        Annotated[LiteralExpr, Tag("literal")],
        Annotated[FunctionExpr, Tag("function")],
    ],
...
class Param(_EndpointModel):
    ...
    default: Optional[Any] = Field(default=None, description="Default value (literal or value expression).")
    ...
    min_length: Optional[int] = Field(default=None, alias="minLength", ge=0)
    max_length: Optional[int] = Field(default=None, alias="maxLength", ge=0)
    min_items: Optional[int] = Field(default=None, alias="minItems", ge=0)
    max_items: Optional[int] = Field(default=None, alias="maxItems", ge=0)
    operators: Optional[list[Literal[
    ...
    controlled_by: Optional[Literal["pagination", "replication"]] = Field(
        default=None,
        description="Marks the param as owned by pagination or replication.",
    )
    style: Optional[str] = Field(default=None, description="OpenAPI query serialization style.")
...
class PageSize(_EndpointModel):
    ...
    default: Optional[Any] = Field(default=None, description="Default page size value or expression.")
...
class OffsetCursor(_EndpointModel):
    ...
    increment_by: Optional[Any] = Field(
        default=None,
...
class PageCursor(_EndpointModel):
    ...
    increment_by: Optional[Any] = Field(default=None, description="Increment per page (defaults to 1).")
...
class OffsetPagination(_EndpointModel):
    ...
    limit: Optional[PageSize] = Field(default=None)
...
Pagination = Annotated[
    Union[
        Annotated[OffsetPagination, Tag("offset")],
        Annotated[PagePagination, Tag("page")],
        Annotated[CursorPagination, Tag("cursor")],
        Annotated[LinkPagination, Tag("link")],
        Annotated[KeysetPagination, Tag("keyset")],
    ],
    Discriminator("type"),
]
...
def _predicate_discriminator(v: Any) -> Optional[str]:
...
Predicate = Annotated[
    Union[
        Annotated[PredEq, Tag("eq")],
        Annotated[PredNeq, Tag("neq")],
        Annotated[PredLt, Tag("lt")],
        Annotated[PredLte, Tag("lte")],
        Annotated[PredGt, Tag("gt")],
        Annotated[PredGte, Tag("gte")],
        Annotated[PredExists, Tag("exists")],
        Annotated[PredMissing, Tag("missing")],
        Annotated[PredEmpty, Tag("empty")],
        Annotated[PredNotEmpty, Tag("not_empty")],
        Annotated[PredAnd, Tag("and")],
        Annotated[PredOr, Tag("or")],
        Annotated[PredNot, Tag("not")],
    ],
...
# and/or/not branches.
PredAnd.model_rebuild()
PredOr.model_rebuild()
PredNot.model_rebuild()
...
class SingleCursorMapping(_EndpointModel):
    ...
    format: Optional[Literal["date-time", "date", "epoch_seconds", "epoch_milliseconds"]] = Field(default=None)
...
def _cursor_mapping_discriminator(v: Any) -> Optional[str]:
...
CursorMapping = Annotated[
    Union[
        Annotated[SingleCursorMapping, Tag("single")],
        Annotated[WindowCursorMapping, Tag("window")],
    ],
...
class _RequestBase(_EndpointModel):
    """Common request fields shared by read and write operations.

    Spec: the published Analitiq schema documentation
    §Request Ownership and §Request Parameter Binding.
    """

    transport_ref: Optional[str] = Field(
        default=None,
        description="Named transport this operation dispatches through; defaults to `default_transport`.",
    )
    path: str = Field(..., min_length=1, description="Path or relative URL on the selected transport.")
    ...
    headers_remove: Optional[list[str]] = Field(
        default=None,
    ...
    def _validate(self) -> "_RequestBase":
        ...
        if self.headers and self.headers_remove:
            declared = {h.lower() for h in self.headers.keys()}
            removed = {h.lower() for h in self.headers_remove}
            overlap = sorted(declared & removed)
            if overlap:
                raise ValueError(
                    f"request.headers_remove must not delete headers declared by the same "
                    f"operation: {overlap!r} (spec: §Request Ownership)"
                )
        return self


class ReadRequest(_RequestBase):
    """Provider request for an API read operation.

    Spec: the published Analitiq schema documentation §Request Ownership.
    """

    method: Literal["GET", "POST"] = Field(..., description="Read HTTP method (closed v1 enum).")

    @model_validator(mode="after")
    def _get_no_body(self) -> "ReadRequest":
        if self.method == "GET" and self.body is not None:
            raise ValueError(
                "read GET operations must not declare request.body "
                "(spec: §Request Bodies)"
            )
        return self
...
def _validate_arrow_type_in_json_schema(
    ...
    # `k2m.models.shared.arrow_shape.enforce_container_shape` runs after
...
class ResponseExtraction(_EndpointModel):
    ...
    Spec: the published Analitiq schema documentation §API Response Extraction.
    """

    records: RefExpr = Field(
        ...,
        description=(
    ...
    metadata: Optional[dict[str, Expression]] = Field(  # type: ignore[valid-type]
        default=None,
...
class WriteError(_EndpointModel):
    ...
    code: Optional[Expression] = Field(default=None)  # type: ignore[valid-type]
    message: Optional[Expression] = Field(default=None)  # type: ignore[valid-type]
    details: Optional[Expression] = Field(default=None)  # type: ignore[valid-type]
...
class WriteResponse(_EndpointModel):
    ...
    Spec: the published Analitiq schema documentation §API Write Response Contract.
    """

    success_when: Optional[Predicate] = Field(default=None)  # type: ignore[valid-type]
    error: Optional[WriteError] = Field(
        default=None,
        description="Optional `code`/`message`/`details` value expressions for failure parsing.",
    )
    affected_records: Optional[Expression] = Field(default=None)  # type: ignore[valid-type]
    generated_keys: Optional[Expression] = Field(default=None)  # type: ignore[valid-type]
    metadata: Optional[dict[str, Expression]] = Field(default=None)  # type: ignore[valid-type]
...
class ReadOperation(_EndpointModel):
    ...
    pagination: Optional[Pagination] = Field(default=None)  # type: ignore[type-arg]
    replication: Optional[Replication] = Field(default=None)
...
def is_valid_conflict_keys(value: Any) -> bool:
    ...
    documents. The untyped read (endpoint-discovery) and ingest (DIP webhook)
...
class WriteOperation(_EndpointModel):
    ...
    conflict_keys: Optional[list[Annotated[str, Field(min_length=1)]]] = Field(
        default=None,
        min_length=1,
        description=(
    ...
    batching: Optional[Batching] = Field(default=None)
    idempotency: Optional[Idempotency] = Field(
        default=None,
        description=(
    ...
    response: Optional[WriteResponse] = Field(default=None)
...
class Operations(_EndpointModel):
    ...
    read: Optional[ReadOperation] = Field(default=None)
    write: Optional[dict[WriteMode, WriteOperation]] = Field(
        default=None,
...
def _validate_no_outer_whitespace(value: str | None) -> str | None:
    if value is None:
        return value
...
def _validate_tags_list(value: list[str] | None) -> list[str] | None:
    """Per `common-metadata-fields.md` §Tags: ≤50 entries, 1-64 chars trimmed, unique."""
    if value is None:
        return value
    if len(value) > 50:
        raise ValueError("tags must contain at most 50 entries (spec: §Shared Metadata)")
    seen: set[str] = set()
    for t in value:
        if not isinstance(t, str):
            raise ValueError("tags must be strings (spec: §Shared Metadata)")
        if t != t.strip():
            raise ValueError("tags must not have leading or trailing whitespace")
        if not (1 <= len(t) <= 64):
            raise ValueError("each tag must be 1-64 chars (spec: §Shared Metadata)")
        if t in seen:
            raise ValueError(f"tags must be unique; duplicate {t!r}")
        seen.add(t)
    return value
...
class _EndpointBase(_EndpointModel):
    ...
    @model_validator(mode="after")
    def _validate_metadata(self) -> "_EndpointBase":
        _validate_no_outer_whitespace(self.display_name)
        _validate_tags_list(self.tags)
        return self
...
class DatabaseObject(_EndpointModel):
    ...
    catalog: Optional[str] = Field(default=None, min_length=1)
    schema_: Optional[str] = Field(default=None, alias="schema", min_length=1)
...
class ColumnFieldSpec(_EndpointModel):
    ...
    properties: Optional[dict[str, "ColumnFieldSpec"]] = Field(default=None)
    items: Optional["ColumnFieldSpec"] = Field(default=None)
...
class DatabaseEndpointDoc(_EndpointBase):
    ...
    @model_validator(mode="after")
    def _validate(self) -> "DatabaseEndpointDoc":
        names = [c.name for c in self.columns]
        dupes = sorted(n for n, count in Counter(names).items() if count > 1)
        if dupes:
            raise ValueError(
                f"columns contain duplicate names {dupes!r} (spec: §Cross-Field Validation)"
            )
        positions = [c.ordinal_position for c in self.columns if c.ordinal_position is not None]
        pos_dupes = sorted(p for p, count in Counter(positions).items() if count > 1)
        if pos_dupes:
            raise ValueError(
                f"ordinal_position values must be unique within columns; duplicates={pos_dupes!r} "
                "(spec: §Cross-Field Validation)"
            )
        if self.primary_keys is not None:
            unknown = sorted(set(self.primary_keys) - set(names))
            if unknown:
                raise ValueError(
                    f"primary_keys reference unknown columns {unknown!r} "
                    "(spec: §Cross-Field Validation)"
                )
        return self
...
_VALUE_EXPR_KEYS: frozenset[str] = frozenset(_EXPRESSION_KEYS)
_ALL_EXPR_KEYS: frozenset[str] = _BINDING_KEYS | _VALUE_EXPR_KEYS

# `function` expressions are *not* singletons — per
# the published Analitiq schema documentation §Value Expressions and
# the `FunctionExpr` model, they declare a `function` name plus optional
# argument fields. The shape validator at `_validate_expression_shapes`
# permits exactly the field set declared on `FunctionExpr` itself, so
...
_FUNCTION_EXPR_FIELDS: frozenset[str] = frozenset(FunctionExpr.model_fields.keys())
...
def _validate_expression_shapes(value: Any, where: str) -> None:
    ...
    if isinstance(value, dict):
        present_expr_keys = [k for k in value if k in _ALL_EXPR_KEYS]
        if present_expr_keys:
            ...
            allowed_siblings = (
                _FUNCTION_EXPR_FIELDS if primary == "function" else {primary}
            )
            non_x_others = sorted(
                k for k in value
...
def _has_disallowed_dynamic_refs(value: Any) -> Optional[str]:
...
def _validate_param_wiring(
    ...
    *,
    ...
    _validate_expression_shapes(request.body, "request.body")
...
def _validate_param_binding_uniqueness(
    ...
    refs.extend(_collect_singleton_values(request.body, "from_param"))
    counts = Counter(refs)
...
def resolve_read_record_schema(response: Any, response_schema: Any) -> Any:
    ...
    (``auto-map-fields`` field extraction, ``endpoint-discovery`` arrow_type
    stamping): they MUST target the same fields, so they call one function.
    """
...
def find_record_field_properties(record_schema: Any) -> dict[str, Any] | None:
    ...
    read contract's consumers (``auto-map-fields`` extraction,
    ``endpoint-discovery`` column derivation) so they enumerate identical
    fields. Consumers read type/annotations off these top-level descriptors
