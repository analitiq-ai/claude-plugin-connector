...
`operations.*.request.path`) and is the sole target the engine acts on. Spec:

  * the published Analitiq schema documentation
    §Endpoint Identity Derivation
  * the published Analitiq schema documentation
    §Shared Metadata (``endpoint_id`` is a deterministic handle)
  * the published Analitiq schema documentation (Slug)
...
This is the single source of truth for the derivation: the minting Lambda
(``endpoint-discovery``) and the connection/endpoint validators import it so
they cannot drift.
"""
...
def derive_db_endpoint_id(
    ...
    published ``^[a-z0-9][a-z0-9_-]*$`` pattern.
    """
    if not name:
