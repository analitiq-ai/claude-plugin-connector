...
# Canonical source: analitiq-ai/infrastructure : validator/src/analitiq_connector_validator.py
# This copy is synced on release; direct edits are overwritten. See README.md.
# ==========================================================================
"""Validate an Analitiq connector, endpoint, or type-map document.
...
contract defines — offline, no schema fetch, no drift. The validator adds only
what a single-document model cannot express:
...
- **endpoint id ↔ locator** (`endpoint-id-locator`): an `endpoint_id` equals the
  ...
  cannot collide (io-contracts.md `resources[].key`); a database id from its verbatim
  `database_object` (`slug(schema)__slug(table)[__slug(catalog)]__hash8`, via the
  shared `k2m.models.endpoint_identity`);
...
Output: a single `{"passed": bool, "findings": [...]}` object on stdout. Exit 0
iff `passed` (no error-severity finding); 1 on error findings / unreadable
document; 2 on CLI usage errors.
"""
...
# The models read `DOMAIN` at import for the `$schema` host `Literal`. This tool
# validates PUBLIC connector documents, which always declare the canonical
# `schemas.analitiq.ai` host — so force it, not setdefault: an ambient
# `DOMAIN=analitiq.dev` (common in infra/dev shells) would otherwise make the
# models reject every `schemas.analitiq.ai` document.
os.environ["DOMAIN"] = "analitiq.ai"

# Run-by-path support (the plugin runtime executes this file directly, without a
# pip install): if a sibling `contract-models` package is vendored alongside —
# the published layout `.../validator/src/…` + `.../contract-models/k2m/` — put
# it on the path so `k2m.models` imports without an install. The `pip install`
# path resolves the same models via the declared dependency instead.
_sibling_models = Path(__file__).resolve().parent.parent.parent / "contract-models"
if (_sibling_models / "k2m").is_dir() and str(_sibling_models) not in sys.path:
    sys.path.insert(0, str(_sibling_models))

try:
    from pydantic import TypeAdapter, ValidationError
    from k2m.models.connector import ConnectorConfig
    from k2m.models.endpoints import ApiEndpointDoc, DatabaseEndpointDoc, ENDPOINT_ID_RE
    from k2m.models.endpoint_identity import derive_db_endpoint_id
    from k2m.models.type_map import TypeMapReadDoc, TypeMapWriteDoc
    ...
    from k2m.models.type_map import _ECMA_NAMED_GROUP, _PLACEHOLDER_RE, _to_python_regex
...
def _normalize_native(value: str) -> str:
    """Uppercase the native probe before read-matching, exactly as the runtime
    reader's `k2m.db.type_map.native_key` does — uppercase only, no strip or
    whitespace-collapse, so coverage can't accept a rule the runtime would miss."""
    return value.upper()
...
def _first_match_render(value: str, rules: list, matcher_key: str, render_key: str,
    ...
    Mirrors the runtime reader (`k2m.db.type_map.match_canonical`): the *input*
    is normalized (read maps uppercase the native via `native_key`), but the
    rule's matcher is compared VERBATIM — exact rules are case-sensitive, so a
    lowercase rule native does not match an uppercased input. `normalize` is
    None for write maps (canonical is matched case-sensitively as authored).
    """
...
# JSON-Schema keyword sets that hold sub-schemas (mirrors k2m.models.endpoints):
...
def _flatten_api_locator(path: str) -> str:
    ...
    contract (io-contracts.md `resources[].key`; issue #917 Gap 2(b)): lowercase,
...
def _endpoint_locator_findings(ep_doc: Any) -> list[dict]:
    ...
    (io-contracts.md `resources[].key`; issue #917 Gap 2(b)). Out of scope for
    ...
    if not handle or not ENDPOINT_ID_RE.match(handle):
        return [finding(
...
# derivation is the single source of truth in `k2m.models.endpoint_identity`
# (imported above) — the same module the `endpoint-discovery` minting Lambda mints
...
def _database_endpoint_locator_findings(ep_doc: Any) -> list[dict]:
    ...
    its verbatim `database_object` (`k2m.models.endpoint_identity`)."""
    if not isinstance(ep_doc, dict):
        return []
    endpoint_id = ep_doc.get("endpoint_id")
    ...
    if endpoint_id != expected:
        return [finding(
            ...
            "per k2m.models.endpoint_identity). The verbatim identity stays in "
...
def check_coverage(doc: dict, doc_path: Path | None) -> list[dict]:
    ...
    # release (spec: endpoint-schema-parameterization.md §Shared Metadata). The
...
_CONNECTOR_ADAPTER = TypeAdapter(ConnectorConfig)
...
def _dispatch(doc: Any, doc_path: Path | None, schema_url: str | None = None) -> list[dict]:
    if is_connector_doc(doc):
        findings = _model_findings(doc, _CONNECTOR_ADAPTER)
        findings += check_coverage(doc, doc_path)
        return findings
    if is_api_endpoint_doc(doc):
        findings = _model_findings(doc, _API_ENDPOINT_ADAPTER)
        findings += _endpoint_locator_findings(doc)
        if doc_path is not None:
            findings += _endpoint_filename_findings(doc, doc_path.name)
        return findings
    if is_database_endpoint_doc(doc):
        # No filename check: database endpoints are runtime-materialized snapshots
        # stored under `.../endpoints/{endpoint_id}/schemas/{schema_hash}.json`
        # (the basename is a hash, not `{endpoint_id}.json`), unlike authored
        # api-endpoint files. The id itself IS gated against database_object.
    ...
    if isinstance(doc, dict) and any(k in doc for k in _CONNECTOR_SENTINELS):
        return _model_findings(doc, _CONNECTOR_ADAPTER)
    # Anything else is a document we were asked to validate but cannot identify —
    # that is a validation failure, not a pass.
    return [finding("json-schema", "error", "/",
                    "document does not match any known artifact (connector / api-endpoint / "
                    "database-endpoint / type-map); a connector must declare 'kind', an "
                    "api-endpoint 'operations', a type-map is a JSON array of rules.")]
